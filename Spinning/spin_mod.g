...
local X,Y,U, map, bndX, bndY, bnd, orient, quad2pair, pair2quad,
      indx,bndd,p,pr,INDX,pos,i, j, ij, x, y, a, b, ia, ib, 
      count, BND, ORIEN, M;
...
U:=K!.binaryArray*0;;
for i in [1..Length(U[1][1])] do
if K!.binaryArray[1][1][i]=1 then
U[1][1][i]:=1; fi;
if K!.binaryArray[1][i]=1 then
U[1][i]:=1; fi;
od;
U:=PureCubicalComplex(U);;
inc:=HAP_PureCubicalPairToCWMap(K,U);
