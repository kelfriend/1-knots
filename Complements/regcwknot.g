...
# UPDATE: needed to account for configuration of corners at the end-step (i.e. 
# when matching the loops of one layer to the other).
# There are sometimes disparities in the ordering on 1-cells from left-to-right
# vs. when ordering from top-to-bottom. Not realising this was causing 111 of
# the pre-stored knots in HAP to yield incorrect CW-decomposition.
        correction:=[];
        for i in [1..len] do
            for j in [1..len] do
...
arc:=ArcPresentation(PureCubicalKnot(6,3));; # one of the problem knots
